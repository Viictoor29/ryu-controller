...
class SDNRestController(ControllerBase):
    ...
    @route("mininet_create_host", "/api/mininet/hosts", methods=["POST", "OPTIONS"])
    def mininet_create_host(self, req, **kwargs):
        if req.method == "OPTIONS":
            return cors_preflight()
        ...
        try:
            ...
            result = self.sdn_app.mininet_service.create_host(
                name=body["name"],
                ip=body.get("ip"),
                mac=body.get("mac")
            )
            ...
            self.sdn_app.logger.exception("Error en POST /api/mininet/hosts: %s", e)
            return error_response(e, status=400)

    @route("mininet_delete_host", "/api/mininet/hosts", methods=["DELETE", "OPTIONS"])
    def mininet_delete_host(self, req, **kwargs):
        if req.method == "OPTIONS":
            return cors_preflight()
        ...
        try:
            ...
            result = self.sdn_app.mininet_service.delete_host(body["name"])
            return success_response(result)
        except Exception as e:
            self.sdn_app.logger.exception("Error en DELETE /api/mininet/hosts: %s", e)
            return error_response(e, status=400)

    @route("mininet_create_switch", "/api/mininet/switches", methods=["POST", "OPTIONS"])
    def mininet_create_switch(self, req, **kwargs):
        if req.method == "OPTIONS":
            return cors_preflight()
        ...
        try:
            ...
            result = self.sdn_app.mininet_service.create_switch(
                name=body["name"],
                protocols=body.get("protocols", "OpenFlow13")
            )
            return success_response(result)
        except Exception as e:
            self.sdn_app.logger.exception("Error en POST /api/mininet/switches: %s", e)
            return error_response(e, status=400)

    @route("mininet_delete_switch", "/api/mininet/switches", methods=["DELETE", "OPTIONS"])
    def mininet_delete_switch(self, req, **kwargs):
        if req.method == "OPTIONS":
            return cors_preflight()
        ...
        try:
            ...
            result = self.sdn_app.mininet_service.delete_switch(body["name"])
            return success_response(result)
        except Exception as e:
            self.sdn_app.logger.exception("Error en DELETE /api/mininet/switches: %s", e)
            return error_response(e, status=400)

    @route("mininet_create_link", "/api/mininet/links", methods=["POST", "OPTIONS"])
    def mininet_create_link(self, req, **kwargs):
        if req.method == "OPTIONS":
            return cors_preflight()

        try:
            body = read_json_body(req)
            require_fields(body, "node1", "node2")

            result = self.sdn_app.mininet_service.create_link(
                node1=body["node1"],
                node2=body["node2"],
            ...
            self.sdn_app.logger.exception("Error en POST /api/mininet/links: %s", e)
            return error_response(e, status=400)

    @route("mininet_delete_link", "/api/mininet/links", methods=["DELETE", "OPTIONS"])
    def mininet_delete_link(self, req, **kwargs):
        if req.method == "OPTIONS":
            return cors_preflight()
        ...
        try:
            ...
            result = self.sdn_app.mininet_service.delete_link(
                node1=body["node1"],
                node2=body["node2"],
                all_links=body.get("all_links", False)
            )
            return success_response(result)
        except Exception as e:
            self.sdn_app.logger.exception("Error en DELETE /api/mininet/links: %s", e)
            return error_response(e, status=400)